...
function initResizer(resizerId, panelId, direction, minPx, maxPx, storageKey) {
  ...
  resizer.addEventListener('mousedown', e => {
    ...
    startSize = direction === 'h' ? panel.getBoundingClientRect().width
  ...
  document.addEventListener('mouseup', () => {
    ...
    // persist
    const saved = direction === 'h'
      ? panel.getBoundingClientRect().width
      : panel.getBoundingClientRect().height;
    try { localStorage.setItem(storageKey, Math.round(saved)); } catch (e) { }
  });
}
...
function restorePanelSizes() {
  const specs = [
    ...
    { panelId: 'node-details-panel', dir: 'v', key: 'lm_intel_h', min: 80, max: 500 },
  ];
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initResizer('intel-resizer', 'node-details-panel', 'v', 80, 500, 'lm_intel_h');
});
...
// ── Tree renderer ────────────────────────────────────────────────────────────
const STATUS_COLOR = {
  running: 'var(--amber)', success: 'var(--teal)', completed: 'var(--teal)',
  failure: 'var(--red)', error: 'var(--red)', flag_found: 'var(--cyan)',
  default: 'var(--border-hi)'
};
const STATUS_FILL = {
  success: 'rgba(30,138,110,.1)', completed: 'rgba(30,138,110,.1)',
  failure: 'rgba(217,64,64,.08)', error: 'rgba(217,64,64,.08)',
  flag_found: 'rgba(0,200,212,.08)', running: 'rgba(240,165,0,.06)', default: '#111418'
};
...
function renderTree({ nodes, rootIds, width, height }) {
  ...
  let html = '<defs><style>.tree-node-rect{cursor:pointer;transition:filter .18s}.tree-node-rect:hover{filter:brightness(1.25)}</style></defs>';
...
function renderConfidence(conf, containerId = 'confidence-list') {
  ...
  el.innerHTML = entries.map(([id, score]) => {
    ...
    const color = pct >= 50 ? 'var(--cyan)' : pct >= 20 ? 'var(--teal)' : pct > 0 ? 'var(--amber)' : 'var(--text-dim)';
    const tag = pct >= 10
